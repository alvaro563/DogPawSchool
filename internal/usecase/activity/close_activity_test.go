...
func closeFinishedActivity(id int) *domain.Activity {
	return domain.MustNewActivity(id, "Paseo", "", "Central", domain.TypeRoute, 5, 1, fixedNow.Add(-25*time.Hour), nil, nil)
}
...
func closeFinishedClosedActivity(id int) *domain.Activity {
	activity, err := domain.ReconstituteActivity(id, "Paseo", "", "Central", domain.TypeRoute, 5, 1, fixedNow.Add(-25*time.Hour), true, nil, nil)
	if err != nil {
		panic(err)
	}
	return activity
...
func closeOngoingActivity(id int) *domain.Activity {
	return domain.MustNewActivity(id, "Paseo", "", "Central", domain.TypeRoute, 5, 2, fixedNow.Add(-30*time.Minute), nil, nil)
}
