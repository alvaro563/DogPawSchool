...
import { Calendar, MapPin, Users, School } from 'lucide-react';
  ...
  return (
    <div className="px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">{activities.length} actividades programadas</p>
      </div>
      ...
      {activities.length === 0 ? (
        ...
        <div className="space-y-2">
          {activities.map((a) => {
            ...
            return (
              <div key={a.id} className={`flex flex-col gap-2 rounded-xl border border-border bg-card p-4 sm:flex-row sm:items-center sm:justify-between ${a.closed ? 'opacity-50' : ''}`}>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-semibold">{a.name}</p>
                    {a.closed && <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">Cerrada</span>}
                  </div>
                  <div className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
                    <span className="inline-flex items-center gap-1"><School className="h-3 w-3" />{TYPE_LABELS[a.activity_type] || a.activity_type}</span>
                    <span>·</span>
                    <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{new Date(a.date).toLocaleDateString('es-ES', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>
                    <span>·</span>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <div className="flex items-center gap-1.5">
                    <div className="h-2 w-16 overflow-hidden rounded-full bg-muted">
                      <div className={`h-full rounded-full ${pct >= 100 ? 'bg-red-500' : pct >= 70 ? 'bg-amber-500' : 'bg-sky-500'}`} style={{ width: `${pct}%` }} />
                    </div>
